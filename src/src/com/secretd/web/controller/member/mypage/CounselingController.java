...
@WebServlet("/member/mypage/counseling")
public class CounselingController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (_memberId == null) // 1. 로그인 한 적이 없다면 일단 로그인 하러가쟈
			...
			request.getRequestDispatcher("/WEB-INF/views/member/mypage/counseling.jsp").forward(request,
					response); // 이어서 출발
		}
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		response.sendRedirect("counseling?writer_id=" + memberId);
	}
}
